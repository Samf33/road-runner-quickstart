...
@Autonomous
public class MABYEAUTOBLUE extends LinearOpMode {
    MecanumDrive drive;
    double[] targets = new double[3];
    double[] irl = new double[3];
    int shotNum = 2;
    Vector2d TARGET_POS = V(-64, -64);
    ...
    final Pose2d SHOOT_POS = P(-17, 17, 3 * Math.PI / 4);
    final Pose2d SHOOT_POS_LONG = P(60, 20, Math.toRadians(150));

    boolean FLIP = true; // true = red, false = blue

    Vector2d V(double x, double y) {
        return FLIP ? new Vector2d(-x, y) : new Vector2d(x, y);
    }

    Pose2d P(double x, double y, double heading) {
        return FLIP
                ? new Pose2d(-x, y, Math.PI - heading)
                : new Pose2d(x, y, heading);
    }

    double H(double heading) {
        return FLIP ? Math.PI - heading : heading;
    }
    ...
    public void runOpMode() throws InterruptedException {
        ...
        drive = new MecanumDrive(hardwareMap, P(60, 20, Math.toRadians(180)));
        waitForStart();

        Action turn = drive.actionBuilder(P(60, 20, Math.toRadians(180))
        ...
        Action getFirstRow = drive.actionBuilder(
                        P(60, 20, Math.toRadians(180) - Math.PI / 6)
                )
                .strafeToLinearHeading(V(28, 15), H(Math.PI / 2))
                .strafeTo(V(28, 68))
                .build();
        Action getSecondRow = drive.actionBuilder(
                ...
                .setReversed(false)
                .strafeToLinearHeading(V(6, 13), H(Math.PI / 2))
                .strafeTo(V(6, 69))
                .build();
        Action getThirdRow = drive.actionBuilder(
                        P(-17, 17, 3 * Math.PI / 4)
                )
                .strafeToLinearHeading(V(-17, 13), H(Math.PI / 2))
                .strafeTo(V(-17, 69))
                .build();
        Action have = new InstantAction(() -> {
            drive.updatePoseEstimate();
            drive.localizer.update();
            AimingUtil.storedPose = P(-drive.localizer.getPose().position.x, drive.localizer.getPose().position.y, -drive.localizer.getPose().heading.toDouble());
        });
        Action wait = drive.actionBuilder(P(-13, 13, 3 * Math.PI / 4))
                .waitSeconds(1)
                .build();
        Action goToLaunch1 = drive.actionBuilder(
                ...
                .setReversed(true)
                .splineToLinearHeading(P(-17, 17, 3 * Math.PI / 4), new Rotation2d(H(-Math.PI / 4), H(-Math.PI / 4)))
                .build();
        ...
        Action goToLaunch3 = drive.actionBuilder(
                ...
                .setReversed(true)
//                .splineToLinearHeading(P(-10, 10, 3*Math.PI/4), new Rotation2d(-Math.PI/4, -Math.PI/4))
                .strafeToLinearHeading(V(-17, 17), H(3 * Math.PI / 4))
                .build();
...
//                .splineTo(V(14, 22), Math.toRadians(160))
//                .waitSeconds(1)
//                .splineTo(V(8, 52), Math.toRadians(180))
//                .waitSeconds(1)
//                .build());
    }
    public void power() {
